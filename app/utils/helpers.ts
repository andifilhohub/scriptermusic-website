...
export const getAge = () => {
  const now = new Date();
  const birth = new Date("March 24, 2000 00:00:00");
  const diff = Math.abs(Number(now) - Number(birth));
  const age = Math.floor(diff / (1000 * 60 * 60 * 24 * 365.25));
  return age;
};

export const getAgeWork = () => {
  const now = new Date();
  const start = new Date("January 01, 2013 00:00:00");
  const diff = Math.abs(Number(now) - Number(start));
  const res = Math.floor(diff / (1000 * 60 * 60 * 24 * 365.25));
  return res;
