...
const moreTracks = computed(() => {
  return isReleaseBootleg(track.type!)
    .filter(
      (el) =>
        Number(new Date(el.date as string)) <= Date.now() &&
        el.id !== id &&
        el.id
    )
    .slice(0, 8);
...
const fanlinkKeys = track?.fanlinks
  ? (Object.keys(track.fanlinks).sort(
      ...
      <div class="container text-secondary text-center pt-3 pb-4 px-3">
        ...
        <div id="track-info" class="row mt-4 mb-4 pt-3 bg-dark">
          <div class="col-12 col-lg-5 info text-center mb-3 px-3">
            <img
              class="img-fx img-fluid release-color-covers"
              :src="`/images/releases/${track.year}/${track.cover}.jpg`"
              :alt="`${track.artists} - ${track.title}`"
            />
          </div>
          <div class="track-info-tags col-12 col-lg-7 meta text-justify px-3">
            <div class="tags">
              <span v-if="track.description">
                <h5 class="mb-0">
                  <span class="meta-heading mb-0 text-white">Description:</span>
                </h5>
              ...
              <span v-if="track.genre">
                <h5 class="mb-0">
                  <span class="meta-heading mb-0 text-white">Genre:</span>
                </h5>
                <h6 class="mb-2">
              ...
              <span v-if="track.album">
                <h5 class="mb-0">
                  <span class="meta-heading mb-0 text-white">Album:</span>
                </h5>
                <h6 class="mb-2">
                  <span class="tag">{{ track.album }}</span>
                </h6>
              </span>
              <span v-if="track.label">
                <h5 class="mb-0">
                  <span class="meta-heading mb-0 text-white">Label:</span>
                </h5>
                <h6 class="mb-2">
              ...
              <span v-if="track.date">
                <h5 class="mb-0">
                  <span class="meta-heading mb-0 text-white"
                    >Release date:</span
                  >
                </h5>
                <h6 class="mb-2">
                  <span class="tag">{{ dateFormat(track.date) }}</span>
                </h6>
              </span>
              <span v-if="track.duration">
                <h5 class="mb-0">
                  <span class="meta-heading mb-0 text-white">Duration:</span>
                </h5>
                <h6 class="mb-2">
              ...
              <span v-if="track.fanlink">
                <h5 class="mb-0">
                  <span class="meta-heading mb-0 text-white">Fanlink:</span>
                </h5>
                <h6 class="mb-2">
                  <span
                    ><a
                      class="tag d-inline-flex align-items-center"
                      target="_blank"
                      :href="`https://${track.fanlink}`"
                      >{{ track.fanlink
                      }}<Icon
                        class="external-link ms-1 fa-fw"
                        name="ri:external-link-line" /></a
                  ></span>
                </h6>
              </span>
              <div
                id="platforms"
                class="mt-2 mb-1 d-flex flex-wrap justify-content-left"
              >
                <template v-for="(platform, i) of fanlinkKeys" :key="i">
                  <div
                    v-if="
                      track.fanlinks &&
                      track.fanlinks[platform] &&
                  ...
                  >
                    <a
                      ...
                      :href="track.fanlinks[platform]"
                      target="_blank"
                      :title="platformMatch(platform)?.title"
                      :style="{
                        background: platformMatch(platform)?.background,
                        color: platformMatch(platform)?.color + '!important',
                      }"
                    >
                      <Icon
                        v-if="platformMatch(platform)?.icon"
                        :name="String(platformMatch(platform)?.icon)"
                        style="font-size: 25px"
                      />
                      <img
                        v-else
                        :src="platformMatch(platform)?.logo"
                        style="height: 25px"
                      />
                    </a>
                  </div>
        ...
        <div id="more_tracks" class="pt-4 text-start">
          <div id="more_releases" class="pb-4">
            <h4 class="m-0 text-white">
              ...
              <a
                class="tag"
                :href="`/releases/${releasePageType(track.type!).type_page}`"
                >{{ releasePageType(track.type!).release_type }}</a
              >
              by Scripter
            </h4>
          </div>
                  ...
                  <NuxtLink :to="`/track/${item.id}`">
                    <div class="overflow-hidden">
                      <img
                        ...
                        :src="`/images/releases/${item.year}/${item.cover}.jpg`"
                        :alt="`${item.artists} - ${item.title}`"
                      />
                    </div>
                    <h5 class="mb-0" style="font-size: 1.25rem">
                      <small
                        ><p class="mb-0 mt-2">{{ item.title }}</p></small
                      >
                    </h5>
                  </NuxtLink>
                  <small
                    ><p class="mb-0">{{ item.artists }}</p></small
                  >
                  <small v-if="item.date"
                    ><p class="mb-4">{{ dateFormat(item.date) }}</p></small
                  >
                </div>
              </div>
            </div>
